(function(){
  ...
  function useGadget(name){
    ...
    if(!g || !g.owned || g.charges<=0) return;

    g.charges -= 1;
    state.profile = StorageAPI.save(state.profile);
    UI.rerenderGadgets();

    playTone(520, 0.10, 0.10);

    if(name === 'X-Ray Goggles' && state.currentProblem && Array.isArray(state.currentProblem.multiChoice)){
      // Remove one wrong option (not random slice).
      const ans = state.currentProblem.answer;
      const opts = state.currentProblem.multiChoice.slice();
      const wrong = opts.filter(o=>String(o).toLowerCase() !== String(ans).toLowerCase());
      if(wrong.length){
        const remove = choice(wrong);
        state.currentProblem.multiChoice = opts.filter(o=>o!==remove);
        UI.showProblem(state.currentProblem, true, false, submitAnswer, requestHint, useGadget, leaveChallenge);
        toast('X-Ray removed one wrong option.', 'good');
      }
    }

    if(name === 'Balance Beam'){
      // Visual hint: show a big mirror panel in the room for a short time.
      state.room.mirrorUntil = performance.now() + 8000;
      UI.appendHint('Balance Beam: keep both sides equal. Undo steps on BOTH sides.');
      toast('Mirror Math activated.', 'good');
    }

    if(name === 'Percent Lens'){
      UI.appendHint('Percent Lens: 10% is divide by 10. 5% is half of 10%.');
      toast('Percent Lens on.', 'good');
    }

    if(name === 'Checkpoint Token'){
      if(!state.room.isBoss){
        UI.appendHint('Checkpoint Token shines… but it only works in boss vaults.');
        return;
      }
      // In boss: restore focus and time a bit.
      state.run.focus = clamp(state.run.focus + 12, 0, 100);
      state.run.timerMs += 3500;
      toast('Checkpoint restored your nerves.', 'good');
    }

    if(name === 'Smoke Bomb'){
      if(state.room.sentry){
        state.room.sentry.disabledMs = 10000;
        toast('Smoke Bomb: sentry blinded for 10s.', 'good');
      } else {
        toast('No sentry in this room.', 'neutral');
      }
    }
  }
  ...
  function completeRoom(){
    ...
    // Completion bonus
    const bonus = room.isBoss ? 8 : 4;
    state.run.gemsRun += bonus;
    spawnBurst(room.terminal.x, room.terminal.y, room.theme.b);
    toast(`Room cleared! +${bonus} gems`, 'good');
    ...
    // Spawn exits for next room choice.
    room.exits = makeExitChoices(room.topic);
    toast(choice(Content.narration.doorChoices), 'neutral');
  }
  ...
  if(window.matchMedia && window.matchMedia('(pointer: coarse)').matches){
    ...
    const updateDir = (ev)=>{
      ...
      touchDir.x = clamp(dx/40, -1, 1);
      touchDir.y = clamp(dy/40, -1, 1);
    };
    ...
    document.getElementById('touch-submit').onclick = ()=>{
      const input = document.getElementById('answer-input');
      if(input) submitAnswer(input.value.trim());
    };
  }
