(function(){
  ...
  function renderProblem(problem, allowChoice, preferFreeResponse){
    ...
    // Timebar is used for timed locks (rhythm, boss pressure, etc.)
    ...
    inputEl.value = '';
    // In rhythm mode, typing is optional but allowed.
  ...
  function drawChoices(options){
    ...
    shuffled.forEach(c=>{
    choicesEl.innerHTML = '';
    // Do not reshuffle in a seeded run by calling Math.random here;
    // options passed in are already shuffled by the generator.
    const opts = [...options];
    opts.forEach(c=>{
  ...
  function renderGadgets(){
    ...
    const state = window.gameState?.profile;
    if(!state){ gadgetRow.innerHTML=''; return; }
    gadgetRow.innerHTML = '';
